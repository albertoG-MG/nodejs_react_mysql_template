...
const user_password_service = require('../../services/validacion/validationCheckPassword');
...
const validateUserData = async (req, res, next) => {
    ...
    if (!Campos.username) {
        ...
        const check_user= await userModel.checarUsuarios(Campos.username);
        if(check_user){
    ...
    if (!Campos.password) {
        ...
        const checar_password = await user_password_service.checarPassword(Campos.password);
        if(checar_password){
            errores.password = `La contraseña contiene ${checar_password} que está en la lista negra. Por favor, escoga otra contraseña`
        }
    }
    ...
    if (!Campos.correo) {
        ...
        const checar_correo = await userModel.checarCorreo(Campos.correo);
        if(checar_correo){
