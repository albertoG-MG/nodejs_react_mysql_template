...
const RolesComponent = ({ isEdit, onRoleSelect }) => {
  const [roles, setRoles] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedRol, setSelectedRol] = useState(isEdit || '');
  ...
  return (
    <div className="grid grid-cols-1 mt-5 mx-7">
        ...
        <div className="group flex">
            ...
            <select className="w-full -ml-10 pl-10 py-2 h-11 border rounded-md border-[#d1d5db] focus:ring-2 focus:ring-indigo-600" name="rol" value={selectedRol}  onChange={handleRoleChange}>
