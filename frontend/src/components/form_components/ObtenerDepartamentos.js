...
const DepartamentosComponent = ({ selectedRoleId }) => {
    ...
    const getDepartamentos = async () => {
      ...
      try {
        ...
        if (error.response && error.response.status === 404) {
            setDepartamentos([]);
            setError('No hay departamentos disponibles para este rol');
        }else{
  ...
  return (
    <div className="grid grid-cols-1 mt-5 mx-7">
        <label className="text-[#64748b] font-semibold mb-2">Departamentos</label>
        <div className="group flex">
            ...
            <select className="w-full -ml-10 pl-10 py-2 h-11 border rounded-md border-[#d1d5db] focus:ring-2 focus:ring-indigo-600" name="departamento">
